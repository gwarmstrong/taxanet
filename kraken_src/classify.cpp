  ...
  if (dna.seq.size() >= Database.get_k()) {
    ...
    while ((kmer_ptr = scanner.next_kmer()) != NULL) {
      ...
      if (scanner.ambig_kmer()) {
        ...
        // TODONE does val_ptr correspond to a kmer or a taxa?
        // ans: should be corresponding to a kmer
        uint32_t *val_ptr = Database.kmer_query(
                              Database.canonical_representation(*kmer_ptr),
                              &current_bin_key,
                              &current_min_pos, &current_max_pos
                            );
        // TODONE figure out what this ternary operation and subsequent operation on hit_counts is
        // looks like hit_counts is a map from taxon to count
        // TODO figure out how to extract taxonomy info from
